...
const ViewReunion = () => {
  ...
  const fetchDemandeDetails = async () => {
    try {
      ...
      setError('Impossible de charger les détails de la réunion');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTraiter = async (demandeId) => {
    if (!window.confirm('Marquer cette réunion comme traitée ?')) return;
    setActionLoading('traiter');
    try {
      await api.put(`/reunion/${demandeId}`, { reunion: 'Traité' });
      await fetchDemandeDetails(); // ✅ Fix 2: await so state updates after fetch
    } catch (error) {
  ...
  const handleRefuser = async (demandeId) => {
    if (!window.confirm('Refuser cette réunion ?')) return;
    setActionLoading('refuser');
    try {
      await api.put(`/reunion/${demandeId}`, { reunion: 'Refusé' });
      await fetchDemandeDetails(); // ✅ Fix 2: await
    } catch (error) {
      console.error('Error updating reunion:', error);
      alert('❌ Erreur lors de la mise à jour du statut.');
    } finally {
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-purple-700 to-purple-800 p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-6xl mx-auto mb-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h1 className="text-3xl sm:text-4xl font-bold text-white">Détails de la réunion</h1>

          {/* ✅ Fix 4: added null fallback at end */}
          {d.reunion == 'Traité' ? (
            ...
            <div className="bg-white/95 backdrop-blur-lg rounded-3xl p-6 shadow-2xl">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-800">Réunion créée</p>
