...
const Reunions = () => {
  ...
  try {
    await api.put(`/reunion/${id}`, { reunion: 'Traité' });
    fetchDemandes();
    setShowPopup(true);
...
const handleRefuser = async (id) => {
  try {
    await api.put(`/reunion/${id}`, { reunion: 'Refusé' });
    fetchDemandes();
    setShowPopup(true);
...
<span>
  ...
  // Statut: En attente - Afficher les boutons d'action
  ...
  // Statut: Traité - Badge vert
  <div className="mt-5 px-5 py-3 bg-[#00d47e] bg-opacity-20 border-2 border-[#00d47e] rounded-full text-center">
    <div className="flex items-center justify-center gap-2 text-[#00d47e] font-semibold">
      <i className="fa-solid fa-circle-check text-lg"></i>
      <span>Demande traitée</span>
    </div>
  </div>
) : demande.reunion === "Refusé" ? (
  // Statut: Refusé - Badge rouge
