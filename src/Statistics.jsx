...
export default function Statistics() {
  ...
  const statsCards = data ? [
    {icon:"fa-building",label:"Entreprises",value: data.total_entreprises,    suffix: ""},
    { icon:"fa-hard-hat",label: "Ouvriers",value: data.total_ouvriers,suffix: ""},
    ...
    {icon:"fa-wallet",label:"Revenus inscriptions",value: data.total_insc,suffix:" DH"},
    {icon:"fa-user-clock",label:"Inscriptions limitées",value:data.inscriptions_limitees, suffix: ""},
  ] : [];
  ...
  const bars = data ? [
    ...
    {
      label: "Taux d'occupation (mise en place / ouvriers)",
      pct: data.total_ouvriers > 0 ? Math.min(100, Math.round((Number(data.total_mise) / Number(data.total_ouvriers)) * 100)) : 0,
      color: "from-fuchsia-400 to-pink-600"
    },
    {
      label: "Inscriptions à renouveler (%)",
      pct: data.total_entreprises > 0 ? Math.min(100, Math.round((Number(data.inscriptions_limitees) / Number(data.total_entreprises)) * 100)) : 0,
